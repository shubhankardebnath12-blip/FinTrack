...
const CustomTooltip = ({ active, payload, currency }) => {
  ...
  const d = payload[0];
  return (
    <div className="px-4 py-3 rounded-2xl animation-scale-in"
      style={{ background: 'linear-gradient(135deg, #0f1520, #0a0f18)', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 20px 40px rgba(0,0,0,0.6)' }}
    >
      <div className="flex items-center gap-2 mb-1">
        <div className="w-2 h-2 rounded-full" style={{ background: d.payload.fill }} />
        <p className="text-xs font-semibold text-white">{d.name}</p>
      </div>
      <p className="text-base font-bold text-white">{formatCurrency(d.value, currency)}</p>
...
const CategoryPieChart = ({ data = [], currency = 'USD' }) => (
  <div className="rounded-2xl p-5 sm:p-6 animate-in"
    style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}
  >
    <div className="mb-5">
      <h3 className="text-sm font-bold" style={{ color: 'var(--color-text)' }}>Spending by Category</h3>
      <p className="text-[11px] mt-0.5" style={{ color: 'var(--color-text-muted)' }}>Expense distribution</p>
    </div>
        ...
        <div className="relative flex-shrink-0">
          <ResponsiveContainer width={180} height={180}>
            <PieChart>
              ...
              <Pie
                ...
                cx={85} cy={85} innerRadius={52} outerRadius={82}
                paddingAngle={3} dataKey="total" nameKey="category" strokeWidth={0}
              >
                {data.slice(0, 8).map((_, i) => <Cell key={i} />)}
              </Pie>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <p className="text-lg font-bold text-white mt-0.5">
              {formatCurrency(data.reduce((s, d) => s + d.total, 0), currency)}
            </p>
          </div>
        </div>
        <div className="flex-1 space-y-2 min-w-0 w-full sm:w-auto">
          {data.slice(0, 6).map((cat, i) => (
            <div key={cat.category} className="flex items-center gap-2.5">
              ...
              <span className="text-xs flex-1 truncate" style={{ color: 'var(--color-text-muted)' }}>{cat.category}</span>
              <span className="text-xs font-semibold" style={{ color: 'var(--color-text)' }}>{cat.percentage}%</span>
            </div>
          ))}
        </div>
      </div>
